...
export async function getRecoveryAnalysis(client: WhoopClient, date?: string): Promise<string> {
  try {
    ...
    const thirtyDaysAgo = new Date(new Date(targetDate).getTime() - 30 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0];
    const historicalRecoveries = await client
      .getRecoveryCollection(`${thirtyDaysAgo}T00:00:00.000Z`, endOfDay)
      .catch(() => []);

    // Calculate 30-day averages for trends
    const avgHRV =
      historicalRecoveries.length > 0
        ? historicalRecoveries.reduce((sum, r) => sum + r.hrv_rmssd_milli, 0) /
          historicalRecoveries.length
        : recovery.hrv_rmssd_milli;

    const avgRHR =
      historicalRecoveries.length > 0
        ? historicalRecoveries.reduce((sum, r) => sum + r.resting_heart_rate, 0) /
          historicalRecoveries.length
